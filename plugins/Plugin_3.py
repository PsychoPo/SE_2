...
class Plugin:
	PluginName = "ContrastedImage"
	PluginDescription = "Плагин позволяет повысить контраст изображения."
	...
	def run(self, img_dir, first):
		...
		contrast_image = contrast.enhance(1.5)

		contrast_image.save(first + '_' + splitext(basename(argv[0]))[0] + '.jpg')
